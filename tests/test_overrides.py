...
class QuantityOverrideTest(unittest.TestCase):
    ...
    def test_convenience_constructors_use_expected_default_units(self):
        ...
        for override, expected in cases.items():
            self.assertEqual(override.value, expected)

    def test_scalar_keeps_integer_representation(self):
        override = Override.scalar("mode_count", 9)

        self.assertEqual(override.value, "9")


class NormalizeOverridesTest(unittest.TestCase):
    ...
    def test_mapping_values_are_stringified(self):
        ...
        self.assertEqual(normalized, {"isp": "320.0", "mode_count": "9"})
